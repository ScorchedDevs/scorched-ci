...
class GithubManager:
    ...
    def create_new_tag_and_release(self, repo, new_tag, release_description):
        ...
        repo.create_git_tag_and_release(tag=new_tag,
    ...
    def replace_changelog_file(self, repo):
        ...
        changelog_file = repo.get_contents("CHANGELOG.md")
        repo.delete_file(changelog_file.path, "Deletando changelog populado", changelog_file.sha, branch="main")
        changelog_template = open('changelog_template.md')
        repo.create_file("CHANGELOG.md", "Recriando changelog a partir do template", changelog_template.read(), branch="main")